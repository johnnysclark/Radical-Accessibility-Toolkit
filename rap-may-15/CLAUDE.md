...
## This folder (rap-may-15)

Lean macOS subset of the toolkit. CONTROLLER lives in `controller/`, WATCHER in `rhino/`, OUTPUT in `output/`, MCP in `mcp/`. No WebUI, no TASC, no Bambu, no laser-cut. See README.md for the demo flow.
...
- Windows aside: run in cmd.exe, not Windows Terminal.
...
- **Exception:** `output/` and `mcp/` are allowed pip dependencies. The controller itself stays stdlib-only.
