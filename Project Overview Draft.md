...
**Image to PIAF — Tactile Image Conversion.** Converts any architectural image — photographs, CAD screenshots, scanned plans, textbook figures, precedent studies — into PIAF-ready output for swell paper printing. Ten presets tuned for different image types (floor plans, elevations, photographs, sketches, sections, site plans, renderings, diagrams, historic photos, hand drawings). Five contrast enhancement methods (auto-contrast, histogram equalization, S-curve, CLAHE, none). Automatic density management ensures swell paper doesn't overload: morphological erosion reduces black pixel density below 30% target while preserving line structure. Grade 1 braille labels rendered directly as dot patterns with overlap detection and abbreviation key generation. Page tiling with registration marks for large images. Output at 300 DPI in PNG, TIFF, or PDF. Also runs as an MCP server (6 tools) for AI-driven conversion workflows. Laser printed on PIAF microcapsule paper with carbon-based toner, then heated — the carbon absorbs heat, the microcapsules swell, and black lines rise off the page. Daniel reads his own floor plans and precedent studies through this pipeline.
