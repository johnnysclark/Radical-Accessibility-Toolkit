// channel-server.ts — Claude Code Channel server for accessible web UI
// Bridges chat messages between the browser and Claude Code via MCP stdio transport.
// Serves index.html, handles chat/edit/script/permission HTTP endpoints, and pushes SSE events.
//
// Three editing paths:
//   PATH 1 (Chat):   POST /chat → Claude → RhinoMCP (sweeping actions, reasoning)
//   PATH 2 (Direct): POST /edit-direct → pending_edits.json → watcher (precision, 0.5s)
//   PATH 3 (Script): POST /script → pending_script.py → watcher exec() (batch, code)
//
// The /edit endpoint still exists and routes edits through Claude (PATH 1 variant).
// Use /edit-direct for immediate watcher-based edits that bypass Claude.
...
const INVENTORY_PATH = resolve(PROJECT_ROOT, "controller/object_inventory.json");
const PENDING_EDITS_PATH = resolve(PROJECT_ROOT, "controller/pending_edits.json");
const PENDING_SCRIPT_PATH = resolve(PROJECT_ROOT, "controller/pending_script.py");
...
try {
  ...
  async fetch(req: Request): Promise<Response> {
    ...
    // -----------------------------------------------------------------------
    // POST /edit-direct — write pending_edits.json for watcher (PATH 2)
    // Bypasses Claude entirely. Watcher picks up edits within 0.5s.
    // -----------------------------------------------------------------------
    if (method === "POST" && url.pathname === "/edit-direct") {
      if (!isAllowedSender(req)) {
        return new Response("ERROR: Sender not allowed", { status: 403 });
      }
      ...
      try {
        writeFileSync(PENDING_EDITS_PATH, JSON.stringify({ edits }, null, 2));
        broadcastSSE({
          type: "status",
          text: `Direct edit queued: ${edits.length} edit(s) sent to watcher.`,
        });
        return new Response(JSON.stringify({ ok: true, count: edits.length, path: "watcher" }), {
          headers: { "Content-Type": "application/json" },
        });
      } catch (err) {
        return new Response(`ERROR: Failed to write pending_edits.json: ${err}`, { status: 500 });
      }
    }

    // -----------------------------------------------------------------------
    // POST /script — write pending_script.py for watcher exec() (PATH 3)
    // Bypasses Claude. Watcher executes the script inside Rhino.
    // -----------------------------------------------------------------------
    if (method === "POST" && url.pathname === "/script") {
      if (!isAllowedSender(req)) {
        return new Response("ERROR: Sender not allowed", { status: 403 });
      }
      ...
      try {
        writeFileSync(PENDING_SCRIPT_PATH, code);
        broadcastSSE({
          type: "status",
          text: "Script queued for Rhino execution.",
        });
        return new Response(JSON.stringify({ ok: true, path: "watcher" }), {
          headers: { "Content-Type": "application/json" },
        });
      } catch (err) {
        return new Response(`ERROR: Failed to write pending_script.py: ${err}`, { status: 500 });
      }
    }

    // -----------------------------------------------------------------------
    // GET /inventory — read object_inventory.json from watcher
    // -----------------------------------------------------------------------
    if (method === "GET" && url.pathname === "/inventory") {
      try {
        ...
        return new Response(`ERROR: Failed to read inventory: ${err}`, { status: 500 });
      }
    }
...
console.error("[webui] MCP server connected via stdio");
