...
from tasc import __version__
from tasc.core.feedback import (
    bay_placed,
    bay_updated,
    corridor_updated,
    describe_model,
...
from tasc.core.validation import validate_site
...
def _get_connector():
    ...
    from tasc.rhino.connector import RhinoConnector
...
def export_cmd(format, output):
    ...
    from tasc.core.exporter import export_3dm, export_piaf, export_text
...
def display_cmd(mode):
    ...
    if not mode:
        # Query current mode
        from tasc.rhino.protocol import execute_script_cmd
        script = (
