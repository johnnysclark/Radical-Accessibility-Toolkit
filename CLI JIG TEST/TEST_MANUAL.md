...
by hand, extending the controller with new commands, and learning
Rhino Python scripting (Mode 3). Each section has step-by-step
instructions and tells you exactly what to expect.
...
introspection tools, and state comparison tools. 48 tools total.
...
as a background process. Claude now has access to all 48 tools.
...
Script Generation (Mode 3: Learning Rhino Python):
46. generate_script - create an editable IronPython script file (v3.2)
47. list_scripts - list all generated scripts (v3.2)
48. show_script - show full script contents (v3.2)


---

# Part 4: Mode 3 — Learning Rhino Python

## What Mode 3 Is

The Layout Jig has three interaction modes:

- Mode 1: Claude Code + MCP. Natural language. Claude translates your intent into model changes. Best for getting started and complex multi-step operations.
- Mode 2: Interactive CLI. Typed commands in a terminal. Direct control. Best for fine-tuning and learning the command vocabulary.
- Mode 3: Rhino Python editor. You write and run IronPython 2.7 scripts that create geometry, query the model, or automate tasks directly inside Rhino. Best for building scripting fluency and doing things the CLI does not cover.

Mode 3 matters because the AI should be a bridge to self-sufficiency. Over time, you learn to write your own scripts. The system generates annotated scripts you can study, modify, and run — building fluency in rhinoscriptsyntax and Python.

## Prerequisites

1. Rhino 8 installed and running
2. The rhino_watcher.py loaded (see Step 6 in Part 1)
3. A screen reader (JAWS or NVDA) active
4. Rhino's EditPythonScript editor (type EditPythonScript in Rhino's command line)

Screen reader tip: EditPythonScript is a standard text editor. JAWS and NVDA can read it line by line. Use Ctrl+Home to go to the top, Ctrl+End to go to the bottom, and standard line navigation.

## Workflow

### Step 1: Ask Claude to Generate a Script

In Claude Code, say something like:

    "Generate a script that draws a circle at each column location in bay A"

Claude calls the generate_script MCP tool, which:
- Validates the code for IronPython 2.7 compatibility (no f-strings, no pathlib)
- Adds a header with instructions and teaching comments
- Saves the file to the scripts/ folder

You will hear a confirmation like:

    OK: Script saved to scripts/draw-column-circles.py
      To run: open draw-column-circles.py in Rhino's EditPythonScript, press F5.
    READY:

### Step 2: Open the Script

Open the file in any text editor you are comfortable with (Notepad, VS Code, or Rhino's EditPythonScript).

To open in Rhino:
1. Type EditPythonScript in Rhino's command line
2. In the editor, use File > Open
3. Navigate to the scripts/ folder in your project directory
4. Select the .py file

### Step 3: Read the Teaching Comments

Every generated script starts with comments explaining what the code does:

    # -*- coding: utf-8 -*-
    # Script: draw-column-circles
    # Draw a circle at each column location in bay A
...
Read through these with your screen reader. They explain each section.

### Step 4: Modify the Script

Change values and see what happens. For example:
- Change a radius from 1.0 to 3.0
- Add a print() statement to check a value
...
In EditPythonScript, press F5 (or click Run).
Output appears in Rhino's command line, which your screen reader can read.

### Step 6: Ask Claude for Help

If something does not work, ask Claude:
- "Why did my script crash on line 12?"
- "How do I draw a rectangle instead of a circle?"
- "Show me the script I wrote earlier" (uses show_script tool)

Claude can read your script with show_script and explain or fix it.

## Example Scripts (Progressive)

### Beginner: Draw a Line

    import rhinoscriptsyntax as rs

    # Draw a line from point (0,0,0) to point (10,5,0)
    start = [0, 0, 0]
    end = [10, 5, 0]
    rs.AddLine(start, end)
    print("Drew a line from {0} to {1}".format(start, end))

What you learn: rs.AddLine(), point coordinates as lists, print() for feedback.

### Intermediate: Read State and Print Room Info

    import json
    import os

    # Find state.json in the same folder as this script
...
What you learn: json.load(), dictionary access, string formatting, loops.

### Advanced: Draw Column Grid from State
    ...
    for col in range(nx + 1):
        for row in range(ny + 1):
            x = ox + col * spacing[0]
            y = oy + row * spacing[1]
            # Draw a small circle at each column
            rs.AddCircle([x, y, 0], 0.75)
            count += 1

    print("Drew {0} column markers for bay A".format(count))

What you learn: nested loops, reading bay geometry, rs.AddCircle(), coordinate math.

## Managing Scripts

List all scripts:

    Claude: "list my scripts"
    (calls list_scripts tool)

Show a script's contents:

    Claude: "show me the draw-column-circles script"
    (calls show_script tool)

Generate a new one:

    Claude: "generate a script that measures the distance between bay A and bay B origins"
    (calls generate_script tool)

## How Mode 3 Connects to Modes 1 and 2

- Mode 1 (Claude) generates scripts via generate_script → you edit and run them
- Mode 2 (CLI) changes state.json → your scripts can read state.json for current data
- Mode 3 (scripts) creates geometry in Rhino → the watcher does not interfere (different layers)
- All three modes share the same state.json as the source of truth

The progression: start with Mode 1 (Claude does everything), move to Mode 2 (you type commands), grow into Mode 3 (you write scripts). The AI is always available to help, but you build independence over time.

## IronPython 2.7 Rules (Important)

Scripts run inside Rhino's IronPython 2.7 interpreter. Key differences from Python 3:

- No f-strings. Use .format() instead: "Hello {0}".format(name)
- No pathlib. Use os.path: os.path.join(a, b)
- print is a function: print("text") works, print "text" also works
- No type hints: def foo(x): not def foo(x: int):
- Dictionary .items() returns a list, not a view

The generate_script tool checks for these automatically and rejects incompatible code.
