...
- TASC CLI (`tasc`): `site`, `grid`, `zone`, `remove`, `list`, `describe`, `export` (piaf/3dm/text), `undo`, `connect`, `run`
- Image Describer CLI (`arch_alt_text.py`): `describe_image`, interactive REPL, batch via `main()`
- Web UI: `channel-server.ts` serving HTTP + SSE with `/chat`, `/edit-direct`, `/script` endpoints
...
## What's essential
- Image-to-PIAF pipeline: threshold, contrast, RainbowTact color-to-pattern, registration marks, auto-scale to paper
...
## What's accidental
- `state_renderer.py` hard-codes Layout Jig schema keys (`bays`, `walls`, `corridors`) — not a generic renderer
- `state_renderer.py` imports `controller/braille.py` via five levels of `os.path.join("..")` — fragile coupling
...
- Web UI hardcodes paths to Layout Jig controller files (`pending_edits.json`, `object_inventory.json`)
- `image_to_piaf` accepts `claude_text_json` override, making Claude both caller and data provider
---
