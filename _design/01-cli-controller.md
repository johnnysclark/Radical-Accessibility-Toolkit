...
- `auditor.py` and `macro_manager.py` are library modules for external callers (MCP, skills)

## Dependencies
- Stdlib: `argparse`, `copy`, `json`, `math`, `os`, `subprocess`, `sys`, `time`, `datetime`
- Optional intra-controller: `template_manager`, `style_manager`, `braille`, `exporter`, `macro_manager` (try/import guarded)
...
## What's essential
- `apply_command` as the single dispatch point — all mutations pass through here for uniform undo, save, and history
...
- Schema migration on `load_state` — old files must upgrade silently without breaking the REPL

## What's accidental
- `controller_cli.py` is 3135 lines; bay, wall, room, corridor, zone, and grid logic are inlined rather than split into modules
- `_default_bay`, `_auto_rooms`, `_init_cells`, and cell-math helpers encode Layout Jig geometry not reusable elsewhere
- `auditor.py` imports `controller_cli as cli` — latent circular dependency
- `macro_manager.py` is disconnected from the REPL; `apply_command` has no `macro` branch; macros only reachable via MCP
- TTS and Bambu/STL auto-export wired into `main()`'s save loop, coupling side-effects to the mutation path
- View rendering helpers (`_view_plan`, `_view_section`, `_view_axon`, etc.) inlined rather than isolated in a renderer module
---
