...
_INTENT_PATH = os.path.join(os.path.dirname(STATE_PATH), "design-intent.md")
_PROGRESS_PATH = os.path.join(os.path.dirname(STATE_PATH), "progress.jsonl")
_EVAL_PATH = os.path.join(os.path.dirname(STATE_PATH), "eval-results.json")


@mcp.tool()
def read_progress() -> str:
    """[Harness] Read design-intent.md, last 5 progress entries, and latest
    eval results. Call at session start for context."""
    parts = []

    # Design intent
    try:
        with open(_INTENT_PATH, "r", encoding="utf-8") as f:
            parts.append("== Design Intent ==\n" + f.read().strip())
    except OSError:
        parts.append("== Design Intent ==\nNo design-intent.md found. "
                      "Ask Daniel what he wants to build, then call "
                      "update_intent to create one.")

    # Progress (last 5 entries)
    try:
        ...
        for line in recent:
            try:
                e = json.loads(line)
                entries.append(
                    f"  {e.get('timestamp', '?')}: {e.get('milestone', '?')} "
                    f"— {e.get('summary', '')}")
    ...
    try:
        ...
        if score is not None:
            ...
            for name, data in criteria.items():
                s = data.get("score", "?")
                n = data.get("notes", "")
                crit_lines.append(f"  {name}: {s}/5 — {n}")
            ...
            parts.append(
                f"== Last Evaluation (score {score}/5) ==\n"
                + "\n".join(crit_lines)
                + ("\n\nIssues:\n" + "\n".join(issue_lines) if issue_lines
                   else ""))
        else:
            parts.append("== Last Evaluation ==\nNo scores recorded yet.")
    except (OSError, json.JSONDecodeError):
        parts.append("== Last Evaluation ==\nNo eval-results.json found.")

    return "OK:\n" + "\n\n".join(parts) + "\nREADY:"


@mcp.tool()
def progress_update(milestone: str, summary: str,
                    bays_modified: str = "") -> str:
    """[Harness] Record a design milestone in progress.jsonl.

    Call after completing a logical unit of work. Also auto-saves a snapshot.

    Args:
        milestone: Short snake_case name (e.g. classroom_wing_complete)
        summary: One-line description of what was accomplished
        bays_modified: Comma-separated bay names that changed (optional)
    """
    ts = _dt.datetime.now().isoformat(timespec="seconds")
    bays_list = [b.strip() for b in bays_modified.split(",") if b.strip()]

    # Auto-save snapshot
    snap_name = milestone.replace("_", "-")
    try:
        state = _load_state()
        cli._snapshot_save(STATE_PATH, state, snap_name)
    except Exception:
        snap_name = "(snapshot failed)"

    entry = {
        "timestamp": ts,
        "milestone": milestone,
        "summary": summary,
        "bays_modified": bays_list,
        "snapshot": snap_name,
    }
    ...
    try:
        ...
        return f"ERROR: Failed to write progress: {e}"

    return (f"OK: Milestone '{milestone}' recorded.\n"
            f"  Snapshot: {snap_name}\n"
            f"  Summary: {summary}\nREADY:")


@mcp.tool()
def evaluate_design(criteria: str = "all") -> str:
    """[Harness] Run adversarial evaluation against design intent.

    Scores 5 criteria 0-5: accessibility, program_fit, circulation,
    tactile_readability, semantic_completeness. Writes to eval-results.json.

    Args:
        criteria: Comma-separated criteria to evaluate, or "all"
    """
    state = _load_state()

    # Gather data
    issues_raw = auditor.audit_model(state)
    audit_text = auditor.format_audit(issues_raw)
    desc = cli.describe(state)
    circ = auditor.describe_circulation(state)

    # Load design intent for comparison
    intent_text = ""
    try:
        with open(_INTENT_PATH, "r", encoding="utf-8") as f:
            intent_text = f.read()
    ...
    ada_issues = [i for i in issues_raw
                  if "width" in i.lower() or "ada" in i.lower()
                  or "door" in i.lower() or "corridor" in i.lower()]
    acc_score = max(0, 5 - len(ada_issues))
    results["accessibility"] = {
        "score": acc_score,
        "notes": f"{len(ada_issues)} ADA-related issues found"
                 if ada_issues else "No ADA issues detected"
    }
    for iss in ada_issues:
        all_issues.append({"severity": "critical", "description": iss})

    # 2. Program fit — zones and areas vs intent
    zone_count = len(zones)
    bay_count = len(bays)
    has_intent = bool(intent_text.strip())
    if not has_intent:
        pf_score = 2
        pf_notes = "No design-intent.md to compare against"
    elif bay_count == 0:
        pf_score = 0
        pf_notes = "No bays created yet"
    else:
        pf_score = min(5, 2 + bay_count)
        pf_notes = f"{bay_count} bays, {zone_count} zones defined"
    results["program_fit"] = {"score": pf_score, "notes": pf_notes}

    # 3. Circulation
    corridor_bays = sum(1 for b in bays.values()
                        if b.get("corridor", {}).get("enabled"))
    door_count = sum(len([a for a in b.get("apertures", [])
                          if a.get("type") == "door"])
                     for b in bays.values())
    if bay_count == 0:
        circ_score = 0
        circ_notes = "No bays to evaluate"
    elif corridor_bays == 0 and bay_count > 1:
        circ_score = 1
        circ_notes = "Multiple bays but no corridors enabled"
        all_issues.append({"severity": "warning",
                           "description": "No corridors connecting bays"})
    else:
        circ_score = min(5, 2 + corridor_bays + (1 if door_count > 0 else 0))
        circ_notes = (f"{corridor_bays} corridor bays, {door_count} doors")
    results["circulation"] = {"score": circ_score, "notes": circ_notes}

    # 4. Tactile readability
    legend = state.get("legend", {})
    legend_entries = len(legend.get("entries", []))
    hatch_set = set()
    for b in bays.values():
        for cell in b.get("cells", {}).values():
            h = cell.get("hatch")
            if h:
                hatch_set.add(h)
    distinct_hatches = len(hatch_set)
    if bay_count == 0:
        tr_score = 0
        tr_notes = "No geometry to assess"
    else:
        tr_score = min(5, 1 + distinct_hatches + (2 if legend_entries > 0
                                                    else 0))
        tr_notes = (f"{distinct_hatches} distinct hatches, "
                    f"{legend_entries} legend entries")
    results["tactile_readability"] = {"score": tr_score, "notes": tr_notes}

    # 5. Semantic completeness
    named_rooms = len(rooms)
    unnamed_cells = 0
    for b in bays.values():
        for ckey, cell in b.get("cells", {}).items():
            if not cell.get("label") and not cell.get("room"):
                unnamed_cells += 1
    if bay_count == 0:
        sc_score = 0
        sc_notes = "No bays to assess"
    else:
        sc_score = max(0, 5 - unnamed_cells)
        sc_notes = (f"{named_rooms} rooms named, "
                    f"{unnamed_cells} cells without labels")
    if unnamed_cells > 0:
        all_issues.append({
            "severity": "warning",
            "description": f"{unnamed_cells} cells have no label or room assignment"
        })
    results["semantic_completeness"] = {"score": sc_score, "notes": sc_notes}
    ...
    try:
        ...
        with open(tmp, "w", encoding="utf-8") as f:
            ...
            f.flush()
            os.fsync(f.fileno())
        os.replace(tmp, _EVAL_PATH)
    except Exception as e:
        return f"ERROR: Failed to write eval results: {e}"

    # Format output
    lines = [f"OK: Design evaluation — overall score {overall}/5"]
    for name, data in results.items():
        lines.append(f"  {name}: {data['score']}/5 — {data['notes']}")
    if all_issues:
        lines.append("")
        lines.append("Issues:")
        for i, iss in enumerate(all_issues, 1):
            lines.append(f"  {i}. [{iss['severity']}] {iss['description']}")
    lines.append("READY:")
    return "\n".join(lines)


@mcp.tool()
def update_intent(section: str, content: str) -> str:
    """[Harness] Update a section of design-intent.md.

    Args:
        section: "program", "constraints", or "status"
        content: New content for that section (markdown)
    """
    valid = ("program", "constraints", "status")
    if section.lower() not in valid:
        return f"ERROR: Section must be one of: {', '.join(valid)}"

    heading = f"## {section.capitalize()}"

    # Read existing or create template
    try:
        with open(_INTENT_PATH, "r", encoding="utf-8") as f:
            text = f.read()
    except OSError:
    ...
    try:
        ...
        return f"ERROR: Failed to write design intent: {e}"

    return f"OK: Updated '{section}' section of design-intent.md.\nREADY:"


# ── Harness prompt: adversarial review ──────────────────

@mcp.prompt()
def adversarial_review() -> str:
    """Adversarial design review. Loads state, intent, and audit data,
    then frames Claude as a skeptical reviewer who scores conservatively."""
    state = _load_state()
    desc = cli.describe(state)
    audit_issues = auditor.audit_model(state)
    audit_text = auditor.format_audit(audit_issues)
    circ = auditor.describe_circulation(state)
    ...
    try:
        ...
        intent = "(No design intent document found)"

    return (
        "You are a design reviewer, not a collaborator. Your job is to find "
        "problems. Do not praise the work. For each criterion, look for the "
        "worst issue first. Score conservatively — a 5 means zero issues "
        "found, which is rare. If you cannot verify a criterion from the "
        "data provided, score it 2 and explain what information is missing.\n\n"
        "== Design Intent ==\n" + intent + "\n\n"
        "== Current Model ==\n" + desc + "\n\n"
        "== Audit Results ==\n" + audit_text + "\n\n"
        "== Circulation ==\n" + circ + "\n\n"
        "Score each criterion 0-5:\n"
        "1. Accessibility: ADA door widths, corridor widths, turning radii\n"
        "2. Program Fit: Do areas match intent requirements?\n"
        "3. Circulation: Are all zones reachable? Dead ends?\n"
        "4. Tactile Readability: Label overlap? Hatch density? Legend?\n"
        "5. Semantic Completeness: All rooms named? Braille labels?\n\n"
        "For each criterion, state the score, the worst issue, and one fix."
    )
...
if __name__ == "__main__":
    ...
    _real_print(f"Tools: 69+ (v2-v4 + harness)", file=sys.stderr)
